...
- (void)sw_addUtilityButtonWithColor:(UIColor *)color selectedColor:(UIColor *)selectedColor icon:(UIImage *)icon title:(NSString *)title{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.backgroundColor = [UIColor whiteColor];
    [button setTitle:title forState:UIControlStateNormal];
    [button setTitleColor:color forState:UIControlStateNormal];
    [button setTitleColor:selectedColor forState:UIControlStateSelected];
    [button setImage:[icon imageWithColor:color] forState:UIControlStateNormal];
    [button setImage:[icon imageWithColor:selectedColor] forState:UIControlStateSelected];
    button.titleLabel.font = [UIFont systemFontOfSize:16.0];
    button.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    button.imageEdgeInsets = UIEdgeInsetsMake(-10, 10, 10, 0);
    button.titleEdgeInsets = UIEdgeInsetsMake(20, -15, -20, 15);
    [self addObject:button];
}
