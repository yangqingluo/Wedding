...
- (void)rightUtilityButtonHandler:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(tableView:didSwipeRowAtIndexPath:didTriggerRightUtilityButtonWithIndex:)]) {
        [self.delegate tableView:self.containingTableView didSwipeRowAtIndexPath:[self.containingTableView indexPathForCell:self] didTriggerRightUtilityButtonWithIndex:utilityButtonIndex];
    }
}
