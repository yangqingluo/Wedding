...
- (void)config{
    ...
    // 先移动contenView
    self.contentView.transform = CGAffineTransformMakeTranslation(0, self.piker.bounds.size.height);
}
#pragma mark -- 取消
- (void)cancle{
...
- (void)sure{
    if (self.back) {
        self.back([NSString stringWithFormat:@"%.0f", 1000.0 * [self.piker.date timeIntervalSince1970]]);
    }
    [self dismissAnimation];    
}
- (void)show{
